...
const useStyles = createStyles(({css, token, isDarkMode}: any, props: any = {}) => {
    ...
    return {
        ...
        commandBar: css`
            ...
            .table-custom-action { display: inline-flex; align-items: center; }
        ...
        searchGroup: css`
            ...
            .search-field.ant-select {
                flex: 0 0 auto;
                align-items: center;
                width: max-content;
                min-width: ${compact ? 54 : 60}px;
                max-width: calc(100% - ${compact ? 80 : 96}px);
                height: 100%;
                padding-block: 0;
                padding-inline: ${compact ? 9 : 12}px 6px;
                font-size: ${compact ? 11 : 12}px;
                color: ${token.colorTextSecondary};
            }
            ...
            .search-field .ant-select-suffix {
                color: ${token.colorTextQuaternary};
            ...
            .search-input .ant-input-suffix { margin-inline-start: 4px; }
        `,
        toolbarTrigger: css`
            display: inline-flex;
            flex: none;
...
// 标题区背景图形与动画样式。
export const useHeroGraphicStyles = createStyles(({css, token}: any) => ({
    graphic: css`
        width: 100%;
        height: 100%;
        display: block;
